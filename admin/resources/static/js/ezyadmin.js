...
// ============== util ======================
ezyadmin.formatTotalResults = function(totalResults, maxTotalResults) {
    var maxValue = maxTotalResults || ezyadmin.paginationCountLimit || 1000000;
    var textAnswer = ezyadmin.formatNumberWithCommas(totalResults);
...
ezyadmin.formatNumberWithCommas = function(input) {
...
ezyadmin.escapeHtml = function(value) {
    if (!value) {
        return '';
    }
    return String(value)
...
ezyadmin.randomWebEmail = function(inputId, length) {
    var url = new URL(ezyadmin.webUrl);
    var domain = url.hostname;
    if (domain.indexOf('.') < 0) {
        domain += '.com';
    }
    var email = `${ezyadmin.randomString(length || 12)}@${domain}`;
    if (inputId) {
        $(`#${inputId}`).val(email);
    }
    return email;
...
ezyadmin.createUrlWithPreviousScreenParams = function(
    ...
    var sign = uri.indexOf('?') < 0 ? '?' : '&';
    return uri + (params.length ? (sign + params.join('&')) : '');
}
// ============== component ======================
...
ezyadmin.formViewByFormData = new WeakMap();
ezyadmin.formDataToObject = function(formName) {
    ...
    var formView = {};
    ezyadmin.formViewByFormData.set(object, formView);
...
ezyadmin.formatInputNumber = function (e) {
    $(e).val(ezyadmin.formatNumberWithCommas($(e).val()));
}
...
ezyadmin.extractMediaUrlAddNowAsVersion = function(media) {
    var url = ezyadmin.extractMediaUrl(media);
    if (url) {
...
ezyadmin.getCookieSocketAccessToken = function() {
    return ezyadmin.getCookie("adminSocketAccessToken");
}

ezyadmin.getCookieWebManagementAccessToken = function() {
    return ezyadmin.getCookie("adminWebManagementAccessToken");
}

ezyadmin.getSocketAccessTokenAsync = async function() {
    return new Promise((resolve, reject) => {
        ezyadmin.getSocketAccessToken(resolve, reject);
    });
}

ezyadmin.getSocketAccessToken = function(successCallback, errorCallback) {
    $.ajax({
        type: 'GET',
        url: '/api/v1/admins/me/socket-access-token',
...
// ============== messages ======================
ezyadmin.translateMessagesAsync = async function(keys) {
    return new Promise((resolve, reject) => {
        ezyadmin.translateMessages(keys, resolve, reject);
    });
}

ezyadmin.translateMessages = function(
    keys,
    successCallback,
    errorCallback
) {
	$.ajax({
        url: `/api/v1/messages?keys=${keys.join(',')}`,
        type: 'GET'
    })
    .done(function(data) {
        if (successCallback) {
