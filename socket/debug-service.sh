...
CLASSPATH="lib/*:external/lib/*:external/resources:settings:socket/lib/*:socket/settings:$1"
...
if [ -z "${PID}" ]; then
    ...
    java $JAVA_OPTS -cp $CLASSPATH $JAVA_OPTS org.youngmonkeys.ezyplatform.socket.SocketStartup $EZYFOX_SERVER_HOME/settings/socket.properties > /dev/null 2>&1 &
    echo $! > ${PID_FILE}
else
